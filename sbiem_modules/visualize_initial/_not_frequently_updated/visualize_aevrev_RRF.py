...
class Vis():
    def __init__(self, Conditions=Conditions, Medium=Medium, 
        ...
        self.gamma = FaultParams['gamma']
        self.Vc_ = FaultParams['Vc_']
        ...
        if not self.Cutoff:
            for i in range(self.ncell):
                for j in range(len(self.V_plot)):
                    self.tau_ss[i, j] = (self.A[i] * tr.log(10**self.V_plot[j]/(10**self.V_plot[j] + self.Vc[i])) + 
                                         self.gamma[i] * self.Vc_ / (10**self.V_plot[j] + self.Vc_) + self.Phi[i, j])
