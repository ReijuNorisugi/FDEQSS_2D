...
class Onetimestep():
    def __init__(self, delta=FieldVariables['delta'], state=FieldVariables['state'], V=FieldVariables['V'], 
                 ...
                 aev=Conditions['aev'], gamma=FaultParams['gamma'], Vc_=FaultParams['Vc_']):
    ...
    def upt_prv(self, upt):
        ...
        self.tau = self.A * self.tr_log(upt.V / (upt.V + self.Vc)) + self.gamma * self.Vc_ / (upt.V + self.Vc_) + self.phi
