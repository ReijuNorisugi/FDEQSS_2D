...
# This is specialized for communication with multiple GPUs when they are directly connected.
...
class Kernel():
    def __init__(self, Devices, Conditions, Medium, FaultParams, FieldVariables):
        ...
        # Set the fastest seismic velocity.
        ...
        # Coefficients for static kernel with periodic boundary.
        ...
        if not self.rmPB:
            ...
            # Make extended space for long Tw to compute pure dynamic kernel
            # without the contamination by waves propagating from the replicated faults.
            self.Lw = self.lam*0.5 + self.C * self.Tw
            self.n_ext = np.arange(0, self.Lw / self.hcell + 1, 1)
            # Prepare corresponding wavenumber.
            if not self.mirror:
        ...
        if self.Stepper == 'LR':
            ...
            # If stepper is RO, we use half of dtmin for double-half step solutions.
        ...
        self.thist = np.array([np.arange(0, self.num) * self.min_step + self.min_step * 0.5]).T
        self.thist = np.flipud(self.thist) # Flip as delay time.
    ...
    def comp_kernel(self):
        ...
        if self.qd:
            ...
            if self.plot_kernel:
                self.plot()
            # For instantaneous convolution when the time step is larger than Tw.
