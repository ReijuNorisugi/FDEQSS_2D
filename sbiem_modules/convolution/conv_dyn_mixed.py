...
class Convolution():
    def __init__(self, kn=Medium['kn'], mu=Medium['mu'], nu=Medium['nu'], Cs=Medium['Cs'], Cp=Medium['Cp'],
        ...
        if not self.mirror:
            ...
            self.dtype_kernel = tr.float32 # For mized-precision computation.
            self.dtype_mix = tr.complex64
            self.dtype_mix_ = tr.complex128
        else:
            self.dtype_np = np.float64 # You should explicitly choose dtype.
            self.dtype_tr = tr.float64 # You should explicitly choose dtype.
            self.pad = tr.zeros(int(self.Nele*(self.nperi-1)/self.nperi), dtype=self.dtype_tr, device=self.device['device_1'])  # For zero-padding.

        self.dDell_hist_real = np.zeros((len(self.thist), len(self.kn)), dtype=self.dtype_np)    # dDell history (real part).
        self.dDell_hist_imag = np.zeros((len(self.thist), len(self.kn)), dtype=self.dtype_np)    # dDell history (real part).
        self.dDell_hist_real = tr.as_tensor(self.dDell_hist_real, dtype=self.dtype_kernel, device='cpu')
        self.dDell_hist_imag = tr.as_tensor(self.dDell_hist_imag, dtype=self.dtype_kernel, device='cpu')
        
        
        if self.num_GPU == 1:
            self.dDell_hist_real = self.dDell_hist_real.to('cuda:0')
            self.dDell_hist_imag = self.dDell_hist_imag.to('cuda:0')
        elif self.num_GPU == 2:
            self.dDell_hist_real_1, self.dDell_hist_real_2 = tr.chunk(self.dDell_hist_real, 2, dim=1)
            self.dDell_hist_imag_1, self.dDell_hist_imag_2 = tr.chunk(self.dDell_hist_imag, 2, dim=1)
            self.dDell_hist_real_1 = self.dDell_hist_real_1.to('cuda:0', non_blocking=True)
            self.dDell_hist_real_2 = self.dDell_hist_real_2.to('cuda:1', non_blocking=True)
            self.dDell_hist_imag_1 = self.dDell_hist_imag_1.to('cuda:0', non_blocking=True)
            self.dDell_hist_imag_2 = self.dDell_hist_imag_2.to('cuda:1', non_blocking=True)
        elif self.num_GPU == 4:
            self.dDell_hist_real_1, self.dDell_hist_real_2, self.dDell_hist_real_3, self.dDell_hist_real_4 = tr.chunk(self.dDell_hist_real, 4, dim=1)
            self.dDell_hist_imag_1, self.dDell_hist_imag_2, self.dDell_hist_imag_3, self.dDell_hist_imag_4 = tr.chunk(self.dDell_hist_imag, 4, dim=1)
            self.dDell_hist_real_1 = self.dDell_hist_real_1.to('cuda:0', non_blocking=True)
            self.dDell_hist_real_2 = self.dDell_hist_real_2.to('cuda:1', non_blocking=True)
            self.dDell_hist_real_3 = self.dDell_hist_real_3.to('cuda:2', non_blocking=True)
            self.dDell_hist_real_4 = self.dDell_hist_real_4.to('cuda:3', non_blocking=True)
            self.dDell_hist_imag_1 = self.dDell_hist_imag_1.to('cuda:0', non_blocking=True)
            self.dDell_hist_imag_2 = self.dDell_hist_imag_2.to('cuda:1', non_blocking=True)
            self.dDell_hist_imag_3 = self.dDell_hist_imag_3.to('cuda:2', non_blocking=True)
            self.dDell_hist_imag_4 = self.dDell_hist_imag_4.to('cuda:3', non_blocking=True)
        ...
        self.full_size = self.dDell_hist_real.shape[0]
        self.previously_dt_ge_Tw = True

        # Save dynamic kernel after first time interation for efficiency.
        self.K = tr.zeros(len(self.kn), dtype=self.dtype_kernel, device=self.device['device_1'])
    ...
    def comp_kernel(self):
        ...
        if self.qd:
            ...
            self.kernel = self.kernel.to(self.dtype_kernel)  # Transform to tr.float32.
            if self.num_GPU != 0:
                self.send_kernel()
    ...
    def comp_kernel_without_pb(self):
        ...
        if self.plot_kernel:
            self.plot()
        self.kernel_sum = tr.sum(self.kernel, axis=0) # For instantaneous convolution when the time step is larger than Tw.
        self.kernel = self.kernel.to(self.dtype_kernel)
        if self.num_GPU != 0:
            ...
            if dt > self.Tw:
                ...
                if not self.previously_dt_ge_Tw:
                    ...
                    if self.num_GPU == 1 or self.num_GPU == 0:
                        self.kkeep_dDell = self.kkeep_dDell.to(self.dtype_mix)
                        ...
                        self.kkeep_dDell_1, self.kkeep_dDell_2 = self.tr_chunk(self.kkeep_dDell.to(self.dtype_mix), 2, dim=0)
                        ...
                        self.kkeep_dDell_1, self.kkeep_dDell_2, self.kkeep_dDell_3, self.kkeep_dDell_4 = self.tr_chunk(
                            self.kkeep_dDell.to(self.dtype_mix), 4, dim=0)
                ...
                if self.num_GPU == 1:
                    self.keep_dDell = self.keep_dDell.to(self.dtype_mix)
                if self.num_GPU == 2:
                    self.keep_dDell_1, self.keep_dDell_2 = self.tr_chunk(self.keep_dDell.to(self.dtype_mix), 2, dim=0)
                    self.keep_dDell_2 = self.keep_dDell_2.to('cuda:1')
                    self.synchronize()
                elif self.num_GPU == 4:
                    self.keep_dDell_1, self.keep_dDell_2, self.keep_dDell_3, self.keep_dDell_4 = self.tr_chunk(
                        self.keep_dDell.to(self.dtype_mix), 4, dim=0)
        ...
        if dt > self.Tw:
            ...
            if self.index >= 0:
                if self.num_GPU == 1 or self.num_GPU == 0:
                    ...
                    self.K = self.tr_complex(self.K_real, self.K_imag).to(self.dtype_mix_)
                elif self.num_GPU == 2:
                    with self.cuda_stream(self.stream_1):
                    ...
                    self.event_1.synchronize()
                    self.event_2.synchronize()
                    self.K = self.tr_cat((self.K1, self.K2), dim=0).to(self.dtype_mix_)
                    self.synchronize()
                elif self.num_GPU == 4:
                    with self.cuda_stream(self.stream_1):
                        self.K1_real, self.K1_imag = conv_jit_comp(self.kernel_1[0:self.num-self.inst, :,],
                    ...
                    self.K = self.tr_cat((self.K1, self.K2,
                                          self.K3, self.K4), dim=0).to(self.dtype_mix_)
                    self.synchronize()
                return self.K
            else:
                if self.num_GPU == 1 or self.num_GPU == 0:
                    ...
                    self.K = self.tr_complex(self.K_real + self.K_real_, self.K_imag + self.K_imag_).to(self.dtype_mix_)
                elif self.num_GPU == 2:
                    with self.cuda_stream(self.stream_1):
