# -*- coding: utf-8 -*-
"""
Code for analyzing dynamic earthquake sequence simulation.
...
fname = '/mRRF/test/'   # Please type directory name.

# Set the path to the host directory.
current = os.getcwd()
rel = ""

# Choose which to plot. When EQ_num is large, Nuclei is time-consuming.
Disp_V_cmap     = True
Check           = True
Trajectory      = True
Nuclei          = True
...
duration_ = 1.  # year
duration_ = duration_ * yr2sec # Transform to sec
...
########################   Load parameters used in simulation   ########################d
...
plt.savefig('{}Figures{}Disp_V_cmap/colormap_v.eps'.format(rel, fname), bbox_inches='tight')
plt.close()

if Disp_V_cmap:
    if EQ_num == 0:
        plot_num = np.max(out_delta)                    # Displacement included in a single figure.
        plot_range = int(np.max(out_delta)//plot_num)   # Number of figures.
    else:
        plot_range = round(EQ_num//plot_num)              # Number of figures.

    # Plot color map of velocity with displacement snapshots and Lapusta plot.
    ...
    for j in range(plot_range):
        if not sparse:
            ...
            if j == 2:
                plt.savefig('{}Figures{}Disp_V_cmap/Displacement_{}.eps'.format(rel, fname, j))
    ...
    if dense:
        ...
        for i in range(len(id_station)):
            for j in range(len(V_plot)):
                Phi[i][j] = np.sum( n_array*np.log(2.)*(k**2)*((( beta[id_station[i]]*k*Y_bar )/( alpha[id_station[i]]*(10**V_plot[j]) + beta[id_station[i]]*k ))**2) )
                tau_ss[i][j] = ( tauc[id_station[i]] + a[id_station[i]]*sigma[id_station[i]]*
                                np.log((10**V_plot[j])/(10**V_plot[j]+Vc[id_station[i]])) 
        ...
        for stat in range(len(id_station)):
            ...
            plt.xticks([-20, -16, -12, -8, -4, 0], fontsize=13)
            ...
            #if stat == 8 or stat == 12:
            #    plt.savefig('{}Figures{}Trajectory/Trajectory_{}.eps'.format(rel, fname, stat))
...
if Nuclei:
    if sparse:
        ...
        """
        y_ul = round(np.max(np.log10(2*Lf*(Y_bar**2))))
        y_ll = round(np.min(np.log10(2*Lf*(out_state_11**2))))
        ...
        for EQ in range(EQ_num):
            ...
            fig, ax = plt.subplots(figsize=(5, 5))
            for i in range(length):
                ax.plot(np.array([k[0], k[1], k[3], k[7], k[-1]]),
                        np.log10(
                        2*Lf*np.array([out_state_1[int(id_main[EQ, 0])+skip*i, int(ncell/2)],
                                        out_state_2[int(id_main[EQ, 0])+skip*i, int(ncell/2)],
                                        out_state_4[int(id_main[EQ, 0])+skip*i, int(ncell/2)],
                                        out_state_8[int(id_main[EQ, 0])+skip*i, int(ncell/2)],
                                        out_state_11[int(id_main[EQ, 0])+skip*i, int(ncell/2)]
                                        ])**2
                        ),
            ...
            for i in range(length_):
                ax.plot(np.array([k[0], k[1], k[3], k[7], k[-1]]),
                        np.log10(
                        2*Lf*np.array([out_state_1[int(id_main[EQ, 0])-skip_*i, int(ncell/2)],
                                        out_state_2[int(id_main[EQ, 0])-skip_*i, int(ncell/2)],
                                        out_state_4[int(id_main[EQ, 0])-skip_*i, int(ncell/2)],
                                        out_state_8[int(id_main[EQ, 0])-skip_*i, int(ncell/2)],
                                        out_state_11[int(id_main[EQ, 0])-skip_*i, int(ncell/2)]
                                        ])**2
                        ),
            ...
            plt.yticks(np.arange(y_ll, y_ul+2, 2))
            plt.minorticks_on()
        ...
        x = np.log10(np.array([10**0, 10**6]))
        x_ = np.log10(np.array([10**0, 10**3]))
        x__ = np.log10(np.array([10**0, 10**8]))

        y = -1*(x - 6) + np.log10(4*10**(-10))
        y_ = -2*(x_ - 3) + np.log10(4*10**(-10))
        y__ = -(1/2)*(x__ - 8) + 2 + np.log10(4*10**(-10))
...
if Summary:
    ...
    for EQ in range(EQ_num):
        ...
        ax.plot(x, y, lw=2., color='black')#ori_cmap_(norm_size[EQ]))
    ...
    for EQ in range(EQ_num):
        ...
        ax.scatter(EQcatalog[EQ, 0]/yr2sec, np.log10(delta_Gc[EQ]), marker='*', s=200, facecolor='black', #facecolor=ori_cmap_(norm_size[EQ]),
                   edgecolor='gray', linewidth=0.5, zorder=2)
    ax.set_xlabel('Simulation time (year)', fontsize=15)
    ax.set_ylabel(r'Average coseismic slip $\log_{10}(\widebar{u})$ (m)', fontsize=12)
    plt.xticks(fontsize=13)
    plt.yticks(fontsize=13)
    plt.tight_layout()
    plt.savefig('{}Figures{}Summary/MT.png'.format(rel, fname), dpi=600)
    #plt.savefig('{}Figures{}Summary/MT.eps'.format(rel, fname))
    plt.close()
    ...
    fig, ax = plt.subplots(figsize=(5, 5))
    ax.hist(np.log10(delta_Gc), 
            bins=30, facecolor='black', edgecolor='silver', lw=1.5, zorder=2)
    ax.set_xlabel(r'Average coseismic slip $\log_{10}(\widebar{u})$', fontsize=15)
    ...
    #plt.savefig('{}Figures{}Summary/GR.eps'.format(rel, fname))
    plt.close()
    ...
    ax.set_xlabel(r'Average coseismic slip $\log_{10}(\widebar{u})$ (m)', fontsize=15)
    ...
    #ax.plot(10**x, 10**y, color='black', lw=1.5)
    #ax.plot(10**x, 10**y_, color='black', lw=1.5)
    #ax.plot(10**x, 10**y__, color='black', lw=1.5)
    ax.set_xlabel(r'Average coseismic slip $\log_{10}(\widebar{u})$ (m)', fontsize=15)
    ax.set_ylabel(r'Log Radiation energy $E_{\rm{R}}$ (J)', fontsize=15)
    plt.xticks(fontsize=13)
    plt.yticks(fontsize=13)
    ...
    x = np.log10(np.array([1*10**(-3), 3*10**(-1)]))
    y = 1*(x - np.log10(1*10**(-3))) + np.log10(2*10**3)

    x_ = np.log10(np.array([7*10**(-3), 2*10**(-1)]))
    y_ = 2*(x_ - np.log10(7*10**(-3))) + np.log10(2*10**3)

    fig, ax = plt.subplots(figsize=(5, 5))
    x = np.log10(delta_Gc)
    y = np.log10(EQcatalog[:, 5])
    ax.scatter(x, y, facecolor='None', lw=2, s=100, zorder=2, edgecolor='deeppink')
    ax.set_xlabel(r'Log coseismic slip $\log_{10}(\widebar{u})$ (m)', fontsize=15)
