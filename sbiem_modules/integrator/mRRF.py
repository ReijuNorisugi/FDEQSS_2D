...
class Heun():
    def __init__(self, Devices, Conditions, Medium, FaultParams, FieldVariables):
        ...
        self.k3 = (FaultParams['k']**2) * FaultParams['n_array'] * tr.log(_)
        self.V0  = Medium['V0']
    ...
    def guess(self, update, ker, cv, pr, ti, dt, dt_, first, second, store):
        ...
        # Update delta, state, Dell, and tau_ini.
        self.keep_V, self.V = update.second(self, dt)
    ...
    def eval_traction(self):
        if self.Cutoff:
            self.phi = self.tauc + self.C*self.tr_sqrt(self.tr_sum(self.k3 * (self.state**2), axis=1))
            self.tau = self.A * self.tr_log(self.V / (self.V + self.Vc)) + self.phi
        else:
            self.phi = self.tauc + self.C*self.tr_sqrt(self.tr_sum(self.k3 * (self.state**2), axis=1))
