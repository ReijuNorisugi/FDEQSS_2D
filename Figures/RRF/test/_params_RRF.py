...
Frict = 'RRF'    # Choose fault constitutive law. Rate- and roughness-dependent friction (RRF) law is available.
...
H = 5*(10**3)         # Depth for averaging. See Lehner (1981).
...
if not mirror:        # Without mirror.
    ...
    Nele = 2048        # Total cell number for computation.
...
if Frict == 'RRF':
    Lf = 1.e1 # 10m
    D_inv = 4.*(4.e-4)*(np.pi**3)/Lf  # factor 4 \times 10^-4 corresponds to Ohnaka's roughness.
    n_up = 2**11
    n_low = 1
    k_size = int(n_up-n_low+1)
    n_array = np.linspace(n_low, n_up, k_size)
    k_size = len(n_array)
    ...
    dim = -3.
    Y_bar = tr.sqrt(D_inv*(k**dim))    # Y_bar^2 is assumed to be proportional to k^-3 (observation of fractured surface).
    ...
    sigma += 50.e6 # Normal stress is uniform on the fault.
    alpha += 1 / (Dc*ku)
    ...
    lambda0 = ((9.*np.pi/32.) * (mu / ( alpha[int(ncell/2)] * tr.max(c) * sigma[int(ncell/2)] )) * 
    (tr.sqrt(tr.sum( (k**2) * (Yss**2) ))) / tr.sum( (k**3) * (Yss**2) )
    )
...
delta =   tr.zeros_like(xp, dtype=dtype, device=device_1)           # Slip.
...
# State and shear stress are assumed at steady state at the initial.
for i in range(len(xp)):
    state[i, :] += beta[i]*k*Y_bar / (alpha[i]*V[i]+beta[i]*k)

phi = tauc + C*tr.sqrt(tr.sum((k**2)*(state**2), axis=1))
