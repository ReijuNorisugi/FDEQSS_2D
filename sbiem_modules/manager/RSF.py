...
# This is the module which summarizes the functions for managing outputs.
...
class Management():
    ...
    def exe_write(self, wr):
        ...
        if self.snap_EQ or self.sparse:
            ...
            wr.write_sparse(self.reshape(self.to_numpy(self.store_delta), (-1, self.out_sample)),
                            ...
                            self.reshape(self.to_numpy(self.store_state), (-1, self.out_sample)),
        ...
        if self.dense:
            wr.write_station(self.reshape(self.to_numpy(self.store_delta_st), (-1, self.stations)),
                            ...
                            self.reshape(self.to_numpy(self.store_state_st), (-1, self.stations)),
                            self.reshape(self.to_numpy(self.store_Ldyn_st), (-1, self.stations))
                            )
        if self.outerror:
            wr.write_error(self.reshape(self.to_numpy(self.store_error), (-1, 1)),
                        self.reshape(self.store_dt, (-1, 1)))
        self.init_strage()
    ...
    def init_strage(self):
        ...
        # Memories for tensors are re-allocated when storing.
        ...
        # This storing system can be updated.
        ...
        self.store_state    = self.tr_zeros(0, dtype=self.dtype, device=self.device)
        self.store_tau      = self.tr_zeros(0, dtype=self.dtype, device=self.device)
        self.store_Ldyn     = self.tr_zeros(0, dtype=self.dtype, device=self.device)
        ...
        self.store_state_st = self.tr_zeros(0, dtype=self.dtype, device=self.device)
        self.store_Ldyn_st = self.tr_zeros(0, dtype=self.dtype, device=self.device)
    ...
    def snap_sparse(self, result):
        ...
        if self.ndt != self.out_ndt:
            ...
            self.store_state = self.tr_cat((self.store_state, result.state[::self.downsample]), dim=0)
            self.store_tau = self.tr_cat((self.store_tau, result.tau[::self.downsample]), dim=0)


    # Store dense output at the stations.
    def snap_dense(self, result):
        ...
        self.store_state_st = self.tr_cat((self.store_state_st, result.state[self.id_station]), dim=0)
    ...
    # Store outputs, you should somehow change here when adding new output.
    # Somehow I need separate function for RO and LR time steppers.
    def store_LR(self, result, error, ker):
        if self.outerror:
            self.calc_source(result)
        else:
            self.calc_source(result)
        self.flag_EQ(result, ker)
        self.snap_sparse(result)
        ...
        if not self.dense:
            ...
            self.snap_dense(result)


    # This is for constant tiem stepper.
    def store_CS(self, single, ker):
        self.calc_source(single)
        self.flag_EQ(single, ker)
        self.snap_sparse(single)
        if not self.dense:
            pass
        else:
            self.snap_dense(single)
    ...
    # Time stepper for Lapusta scheme.
    def stepper_LR(self, single, double, error, wr, ti, ker, cv, cv_):
        if not self.outerror:
            # Adopt time.
            ti.tev()
            # Update class variables for time.
            self.time_upt(ti)
            # Write outputs.
            self.store_LR(single, error, ker)
            # Update self for the next time step.
            single.upt_prv(single)
            # Adopt time step.
            ti.dtev(single.V)
        else:
            ti.tev()
            self.time_upt(ti)
            cv.align_hist(cv_, False)
            self.store_LR(double, error, ker)
            # Update self.
            single.upt_prv(double)
            single.upt_prs(double)
            double.upt_prv(double)
    ...
    # Time stepper with constant dt.
    def stepper_CS(self, single, wr, ti, ker):
        ti.tev()
        self.time_upt(ti)
        single.upt_prv(single)
        self.store_CS(single, ker)
        if not self.id_count_st % self.tmp_out == 0:
            pass
        else:
            self.exe_write(wr)
