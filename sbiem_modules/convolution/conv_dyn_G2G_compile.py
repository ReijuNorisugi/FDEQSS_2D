...
class Convolution():
    def __init__(self, Devices, Conditions, Medium, FaultParams, FieldVariables):
        ...
        # If 4 GPUs are used, better to increase cache size for compilation.
        if self.num_GPU == 4:
            # Default is 8.
            tr._dynamo.config.cache_size_limit = 16
    ...
    def conv_inst_1(self, kernel_sum, kernel, d_Dell, dt0):
        if dt0 >= self.Tw:
            ...
            return compute_Kinst(kernel[self.num-self.inst:self.num, :]) * d_Dell
    ...
    def conv_inst_2(self, kernel_sum, kernel_1, kernel_2, d_Dell, dt0):
        if dt0 >= self.Tw:
            ...
            with self.cuda_stream(self.stream_1):
                ...
                self.K_inst_1 = compute_Kinst(kernel_1[self.num-self.inst:self.num, :])
                self.event_1.record()
            with self.cuda_stream(self.stream_2):
                #self.K_inst_2 = kernel_2[self.num-self.inst:self.num, :].sum(dim=0)
                self.K_inst_2 = compute_Kinst(kernel_2[self.num-self.inst:self.num, :])
    ...
    def conv_inst_4(self, kernel_sum, kernel_1, kernel_2, kernel_3, kernel_4, d_Dell, dt0):
        if dt0 >= self.Tw:
            ...
            with self.cuda_stream(self.stream_3):
                ...
                self.K_inst_3 = compute_Kinst(kernel_3[self.num-self.inst:self.num, :])
                self.K_inst_3 = self.K_inst_3.to('cuda:0', non_blocking=True)
                self.event_3.record()
            with self.cuda_stream(self.stream_4):
                #self.K_inst_4 = kernel_4[self.num-self.inst:self.num, :].sum(dim=0)
                self.K_inst_4 = compute_Kinst(kernel_4[self.num-self.inst:self.num, :])
        ...
        if dt > self.Tw:
            ...
            if self.split:
                if self.index >= 0:
                    ...
                    self.Kr = compute_K(kernel[0:self.num-self.inst, :], self.dDell_hist_r[self.index:self.latest+1, :])
                    self.Ki = compute_K(kernel[0:self.num-self.inst, :], self.dDell_hist_i[self.index:self.latest+1, :])
                    ...
                    self.Kr = compute_K(kernel[0:-self.index, :], self.dDell_hist_r[self.index+self.num:self.num, :])
                    self.Kr += compute_K(kernel[-self.index:self.num-self.inst, :], self.dDell_hist_r[0:self.latest+1, :])
                    self.Ki = compute_K(kernel[0:-self.index, :], self.dDell_hist_i[self.index+self.num:self.num, :])
                    self.Ki += compute_K(kernel[-self.index:self.num-self.inst, :], self.dDell_hist_i[0:self.latest+1, :])
                    self.K = self.tr_complex(self.Kr, self.Ki)
                    return self.K
            else:
                if self.index >= 0:
            ...
            if self.index >= 0:
                if self.split:
                    with self.cuda_stream(self.stream_1):
                        ...
                        self.K1r = compute_K(kernel_1[0:self.num-self.inst, :], self.dDell_hist_1r[self.index:self.latest+1, :])
                        self.K1i = compute_K(kernel_1[0:self.num-self.inst, :], self.dDell_hist_1i[self.index:self.latest+1, :])
                    ...
                    with self.cuda_stream(self.stream_2):
                        ...
                        self.K2r = compute_K(kernel_2[0:self.num-self.inst, :], self.dDell_hist_2r[self.index:self.latest+1, :])
                        self.K2i = compute_K(kernel_2[0:self.num-self.inst, :], self.dDell_hist_2i[self.index:self.latest+1, :])
                ...
                if self.split:
                    with self.cuda_stream(self.stream_1):
                        ...
                        self.K1r = compute_K(kernel_1[0:-self.index, :], self.dDell_hist_1r[self.index+self.num:self.num, :])
                        self.K1r += compute_K(kernel_1[-self.index:self.num-self.inst, :], self.dDell_hist_1r[0:self.latest+1, :])
                        self.K1i = compute_K(kernel_1[0:-self.index, :], self.dDell_hist_1i[self.index+self.num:self.num, :])
                        self.K1i += compute_K(kernel_1[-self.index:self.num-self.inst, :], self.dDell_hist_1i[0:self.latest+1, :])
                        self.K1 = self.tr_complex(self.K1r, self.K1i)
                        self.event_1.record()
                    with self.cuda_stream(self.stream_2):
                        ...
                        self.K2r = compute_K(kernel_2[0:-self.index, :], self.dDell_hist_2r[self.index+self.num:self.num, :])
                        self.K2r += compute_K(kernel_2[-self.index:self.num-self.inst, :], self.dDell_hist_2r[0:self.latest+1, :])
                        self.K2i = compute_K(kernel_2[0:-self.index, :], self.dDell_hist_2i[self.index+self.num:self.num, :])
                        self.K2i += compute_K(kernel_2[-self.index:self.num-self.inst, :], self.dDell_hist_2i[0:self.latest+1, :])
                        self.K2 = self.tr_complex(self.K2r, self.K2i)
                        self.K2 = self.K2.to('cuda:0', non_blocking=True)
                        self.event_2.record()
                else:    
            ...
            if self.index >= 0:
                if self.split:
                    ...
                    with self.cuda_stream(self.stream_3):
                        ...
                        self.K3r = compute_K(kernel_3[0:self.num-self.inst, :], self.dDell_hist_3r[self.index:self.latest+1, :])
                        self.K3i = compute_K(kernel_3[0:self.num-self.inst, :], self.dDell_hist_3i[self.index:self.latest+1, :])
                    ...
                    with self.cuda_stream(self.stream_4):
                        ...
                        self.K4r = compute_K(kernel_4[0:self.num-self.inst, :], self.dDell_hist_4r[self.index:self.latest+1, :])
                        self.K4i = compute_K(kernel_4[0:self.num-self.inst, :], self.dDell_hist_4i[self.index:self.latest+1, :])
                ...
                if self.split:
                    ...
                    with self.cuda_stream(self.stream_3):
                        ...
                        self.K3r = compute_K(kernel_3[0:-self.index, :], self.dDell_hist_3r[self.index+self.num:self.num, :])
                        self.K3r += compute_K(kernel_3[-self.index:self.num-self.inst, :], self.dDell_hist_3r[0:self.latest+1, :])
                        self.K3i = compute_K(kernel_3[0:-self.index, :], self.dDell_hist_3i[self.index+self.num:self.num, :])
                        self.K3i += compute_K(kernel_3[-self.index:self.num-self.inst, :], self.dDell_hist_3i[0:self.latest+1, :])
                        self.K3 = self.tr_complex(self.K3r, self.K3i)
                        self.K3 = self.K3.to('cuda:0', non_blocking=True)
                        self.event_3.record()
                    with self.cuda_stream(self.stream_4):
                        ...
                        self.K4r = compute_K(kernel_4[0:-self.index, :], self.dDell_hist_4r[self.index+self.num:self.num, :])
                        self.K4r += compute_K(kernel_4[-self.index:self.num-self.inst, :], self.dDell_hist_4r[0:self.latest+1, :])
                        self.K4i = compute_K(kernel_4[0:-self.index, :], self.dDell_hist_4i[self.index+self.num:self.num, :])
                        self.K4i += compute_K(kernel_4[-self.index:self.num-self.inst, :], self.dDell_hist_4i[0:self.latest+1, :])
                        self.K4 = self.tr_complex(self.K4r, self.K4i)
                        self.K4 = self.K4.to('cuda:0', non_blocking=True)
                        self.event_4.record()
                else:
...
def compute_K(kernel, dDell):
    return (kernel * dDell).sum(dim=0)

@tr.compile(dynamic=True)
def compute_Kinst(kernel):
