...
class Convolution():
    ...
    def conv(self, dt):
        ...
        if dt > self.Tw:
            ...
            self.index = self.latest + self.inst - self.num + 1
            if self.index >= 0:
                self.K = conv_jit_comp(self.kernel[0:self.num-self.inst, :], 
                                       self.dDell_hist[self.index:self.latest+1, :])
                return self.K
            else:
                self.K = conv_jit_comp(self.kernel[0:-self.index, :],
                                       self.dDell_hist[self.index+self.num:self.num, :])
                self.K_ = conv_jit_comp(self.kernel[-self.index:self.num-self.inst, :],
