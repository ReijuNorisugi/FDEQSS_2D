...
class Management():
    ...
    def snap_sparse(self, result):
        ...
        if self.ndt != self.out_ndt:
            ...
            self.store_state_1  = self.tr_cat((self.store_state_1, result.state[::self.downsample, int(2**0-1)]), dim=0)
            self.store_state_2  = self.tr_cat((self.store_state_2, result.state[::self.downsample, int(2**2-1)]), dim=0)
            self.store_state_4  = self.tr_cat((self.store_state_4, result.state[::self.downsample, int(2**4-1)]), dim=0)
    ...
    def snap_dense(self, result):
        ...
        self.store_state1_st = self.tr_cat((self.store_state1_st, result.state[self.id_station, int(2**0-1)]), dim=0)
        self.store_state2_st = self.tr_cat((self.store_state2_st, result.state[self.id_station, int(2**2-1)]), dim=0)
        self.store_state4_st = self.tr_cat((self.store_state4_st, result.state[self.id_station, int(2**4-1)]), dim=0)
        self.store_state8_st = self.tr_cat((self.store_state8_st, result.state[self.id_station, int(2**8-1)]), dim=0)
        self.store_state11_st   = self.tr_cat((self.store_state11_st, result.state[self.id_station, -1]), dim=0)
