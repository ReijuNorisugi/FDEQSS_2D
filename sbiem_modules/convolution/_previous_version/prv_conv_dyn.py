...
# This is the module for elastic kernel computation and dynamic convolution.
# Here, we employ periodic boundary condition.
...
class Convolution():
    def __init__(self, kn=Medium['kn'], mu=Medium['mu'], nu=Medium['nu'], Cs=Medium['Cs'], Cp=Medium['Cp'],
        ...
        self.dDell_hist = np.zeros((len(self.thist), len(self.kn)), dtype=self.dtype_np)      # dDell history.
        self.dDell_hist = tr.as_tensor(self.dDell_hist, dtype=self.dtype_tr, device='cpu')  # dDell history.
        if self.num_GPU == 1:
            self.dDell_hist = self.dDell_hist.to('cuda:0')
        elif self.num_GPU == 2:
            self.dDell_hist_1, self.dDell_hist_2 = tr.chunk(self.dDell_hist, 2, dim=1)
            self.dDell_hist_1 = self.dDell_hist_1.to('cuda:0', non_blocking=True)
            self.dDell_hist_2 = self.dDell_hist_2.to('cuda:1', non_blocking=True)
        elif self.num_GPU == 4:
            self.dDell_hist_1, self.dDell_hist_2, self.dDell_hist_3, self.dDell_hist_4 = tr.chunk(self.dDell_hist, 4, dim=1)
            self.dDell_hist_1 = self.dDell_hist_1.to('cuda:0', non_blocking=True)
            self.dDell_hist_2 = self.dDell_hist_2.to('cuda:1', non_blocking=True)
            self.dDell_hist_3 = self.dDell_hist_3.to('cuda:2', non_blocking=True)
            self.dDell_hist_4 = self.dDell_hist_4.to('cuda:3', non_blocking=True)
        elif self.num_GPU == 8:
            self.dDell_hist_1, self.dDell_hist_2, self.dDell_hist_3, self.dDell_hist_4,
            self.dDell_hist_5, self.dDell_hist_6, self.dDell_hist_7, self.dDell_hist_8 = tr.chunk(self.dDell_hist, 8, dim=1)
            self.dDell_hist_1 = self.dDell_hist_1.to('cuda:0', non_blocking=True)
            self.dDell_hist_2 = self.dDell_hist_2.to('cuda:1', non_blocking=True)
            self.dDell_hist_3 = self.dDell_hist_3.to('cuda:2', non_blocking=True)
            self.dDell_hist_4 = self.dDell_hist_4.to('cuda:3', non_blocking=True)
            self.dDell_hist_5 = self.dDell_hist_5.to('cuda:4', non_blocking=True)
            self.dDell_hist_6 = self.dDell_hist_6.to('cuda:5', non_blocking=True)
            self.dDell_hist_7 = self.dDell_hist_7.to('cuda:6', non_blocking=True)
            self.dDell_hist_8 = self.dDell_hist_8.to('cuda:7', non_blocking=True)
        ...
        if self.num_GPU == 2:
            ...
            self.stream_3 = tr.cuda.Stream(device='cuda:2')
            self.stream_4 = tr.cuda.Stream(device='cuda:3')
            self.stream_5 = tr.cuda.Stream(device='cuda:4')
            self.stream_6 = tr.cuda.Stream(device='cuda:5')
            self.stream_7 = tr.cuda.Stream(device='cuda:6')
            self.stream_8 = tr.cuda.Stream(device='cuda:7')
            self.event_1 = tr.cuda.Event()
            self.event_2 = tr.cuda.Event()
            self.event_3 = tr.cuda.Event()
            self.event_4 = tr.cuda.Event()
            self.event_5 = tr.cuda.Event()
            self.event_6 = tr.cuda.Event()
            self.event_7 = tr.cuda.Event()
            self.event_8 = tr.cuda.Event()
    ...
    # Function for transforming ndarryay to tensor.
    def to_tensor(self, array):
        return tr.from_numpy(np.ascontiguousarray(array))
    ...
    def comp_kernel(self):
        ...
        if self.qd:
            ...
            self.kernel = self.to_tensor(self.kernel)
    ...
    def send_kernel(self):
        if self.num_GPU == 1:
            ...
            self.kernel_1, self.kernel_2, self.kernel_3, self.kernel_4,
            self.kernel_5, self.kernel_6, self.kernel_7, self.kernel_8 = tr.chunk(self.kernel, 8, dim=1)
            self.kernel_1 = self.kernel_1.to('cuda:0', non_blocking=True)
            self.kernel_2 = self.kernel_2.to('cuda:1', non_blocking=True)
            self.kernel_3 = self.kernel_3.to('cuda:2', non_blocking=True)
            self.kernel_4 = self.kernel_4.to('cuda:3', non_blocking=True)
            self.kernel_5 = self.kernel_5.to('cuda:4', non_blocking=True)
            self.kernel_6 = self.kernel_6.to('cuda:5', non_blocking=True)
            self.kernel_7 = self.kernel_7.to('cuda:6', non_blocking=True)
            self.kernel_8 = self.kernel_8.to('cuda:7', non_blocking=True)
            self.kernel_sum = self.kernel_sum.to('cuda:0', non_blocking=True)
        ...
        if store:
            ...
            if dt > self.Tw:
                ...
                if not self.previously_dt_ge_Tw:
                    pass
                else: # Repeating this process if dt > Tw is time-consuming.
                    if self.num_GPU == 1 or self.num_GPU == 0:
                        ...
                        self.kkeep_dDell_1, self.kkeep_dDell_2, self.kkeep_dDell_3, self.kkeep_dDell_4,
                        self.kkeep_dDell_5, self.kkeep_dDell_6, self.kkeep_dDell_7, self.kkeep_dDell_8 = self.tr_chunk(self.kkeep_dDell, 8, dim=0)
                        self.kkeep_dDell_2 = self.kkeep_dDell_2.to('cuda:1', non_blocking=True)
                        self.kkeep_dDell_3 = self.kkeep_dDell_3.to('cuda:2', non_blocking=True)
                        self.kkeep_dDell_4 = self.kkeep_dDell_4.to('cuda:3', non_blocking=True)
                        self.kkeep_dDell_5 = self.kkeep_dDell_5.to('cuda:4', non_blocking=True)
                        self.kkeep_dDell_6 = self.kkeep_dDell_6.to('cuda:5', non_blocking=True)
                        self.kkeep_dDell_7 = self.kkeep_dDell_7.to('cuda:6', non_blocking=True)
                        self.kkeep_dDell_8 = self.kkeep_dDell_8.to('cuda:7', non_blocking=True)
                        self.synchronize()
                        ...
                        with self.cuda_stream(self.stream_4):
                            self.dDell_hist_4.copy_(self.kkeep_dDell_4.expand_as(self.dDell_hist_4))
                            self.event_4.record()
                        with self.cuda_stream(self.stream_5):
                            self.dDell_hist_5.copy_(self.kkeep_dDell_5.expand_as(self.dDell_hist_5))
                            self.event_5.record()
                        with self.cuda_stream(self.stream_6):
                            self.dDell_hist_6.copy_(self.kkeep_dDell_6.expand_as(self.dDell_hist_6))
                            self.event_6.record()
                        with self.cuda_stream(self.stream_7):
                            self.dDell_hist_7.copy_(self.kkeep_dDell_7.expand_as(self.dDell_hist_7))
                            self.event_7.record()
                        with self.cuda_stream(self.stream_8):
                            self.dDell_hist_8.copy_(self.kkeep_dDell_8.expand_as(self.dDell_hist_8))
                            self.event_8.record()
                        self.event_1.synchronize()
                        self.event_2.synchronize()
                        self.event_3.synchronize()
                        self.event_4.synchronize()
                        self.event_5.synchronize()
                        self.event_6.synchronize()
                        self.event_7.synchronize()
                        self.event_8.synchronize()
                ...
                if self.num_GPU == 2:
                    ...
                    self.keep_dDell_1, self.keep_dDell_2, self.keep_dDell_3, self.keep_dDell_4,
                    self.keep_dDell_5, self.keep_dDell_6, self.keep_dDell_7, self.keep_dDell_8 = self.tr_chunk(self.keep_dDell, 8, dim=0)
                    self.keep_dDell_2 = self.keep_dDell_2.to('cuda:1', non_blocking=True)
                    self.keep_dDell_3 = self.keep_dDell_3.to('cuda:2', non_blocking=True)
                    self.keep_dDell_4 = self.keep_dDell_4.to('cuda:3', non_blocking=True)
                    self.keep_dDell_5 = self.keep_dDell_5.to('cuda:4', non_blocking=True)
                    self.keep_dDell_6 = self.keep_dDell_6.to('cuda:5', non_blocking=True)
                    self.keep_dDell_7 = self.keep_dDell_7.to('cuda:6', non_blocking=True)
                    self.keep_dDell_8 = self.keep_dDell_8.to('cuda:7', non_blocking=True)
                    self.synchronize()
                else:
                    pass
                ...
                if self.latest == self.num - 1:
                    if self.num_GPU == 1 or self.num_GPU == 0:
                        ...
                        with self.cuda_stream(self.stream_4):
                            ...
                            self.event_4.record()
                        with self.cuda_stream(self.stream_5):
                            self.dDell_hist_5[0:self.inst, :].copy_(self.keep_dDell_5.expand_as(self.dDell_hist_5[0:self.inst, :]))
                            self.event_5.record()
                        with self.cuda_stream(self.stream_6):
                            self.dDell_hist_6[0:self.inst, :].copy_(self.keep_dDell_6.expand_as(self.dDell_hist_6[0:self.inst, :]))
                            self.event_6.record()
                        with self.cuda_stream(self.stream_7):
                            self.dDell_hist_7[0:self.inst, :].copy_(self.keep_dDell_7.expand_as(self.dDell_hist_7[0:self.inst, :]))
                            self.event_7.record()
                        with self.cuda_stream(self.stream_8):
                            self.dDell_hist_8[0:self.inst, :].copy_(self.keep_dDell_8.expand_as(self.dDell_hist_8[0:self.inst, :]))
                            self.event_8.record()
                        self.event_1.synchronize()
                        self.event_2.synchronize()
                        self.event_3.synchronize()
                    ...
                    if self.num_GPU == 1 or self.num_GPU == 0:
                        self.dDell_hist[self.index_:self.index__, :].copy_(self.keep_dDell.expand_as(self.dDell_hist[self.index_:self.index__, :]))
                    elif self.num_GPU == 2:
                        with self.cuda_stream(self.stream_1):
                            self.dDell_hist_1[self.index_:self.index__, :].copy_(self.keep_dDell_1.expand_as(self.dDell_hist_1[self.index_:self.index__, :]))
                            self.event_1.record()
                        with self.cuda_stream(self.stream_2):
                            self.dDell_hist_2[self.index_:self.index__, :].copy_(self.keep_dDell_2.expand_as(self.dDell_hist_2[self.index_:self.index__, :]))
                            self.event_2.record()
                        self.event_1.synchronize()
                        self.event_2.synchronize()
                    elif self.num_GPU == 4:
                        ...
                        with self.cuda_stream(self.stream_2):
                            ...
                            self.event_2.record()
                        with self.cuda_stream(self.stream_3):
                            self.dDell_hist_3[self.index_:self.index__, :].copy_(self.keep_dDell_3.expand_as(self.dDell_hist_3[self.index_:self.index__, :]))
                            self.event_3.record()
                        with self.cuda_stream(self.stream_4):
                            self.dDell_hist_4[self.index_:self.index__, :].copy_(self.keep_dDell_4.expand_as(self.dDell_hist_4[self.index_:self.index__, :]))
                            self.event_4.record()
                        self.event_1.synchronize()
                        self.event_2.synchronize()
                        self.event_3.synchronize()
                        ...
                        with self.cuda_stream(self.stream_4):
                            ...
                            self.event_4.record()
                        with self.cuda_stream(self.stream_5):
                            self.dDell_hist_5[self.index_:self.index__, :].copy_(self.keep_dDell_5.expand_as(self.dDell_hist_5[self.index_:self.index__, :]))
                            self.event_5.record()
                        with self.cuda_stream(self.stream_6):
                            self.dDell_hist_6[self.index_:self.index__, :].copy_(self.keep_dDell_6.expand_as(self.dDell_hist_6[self.index_:self.index__, :]))
                            self.event_6.record()
                        with self.cuda_stream(self.stream_7):
                            self.dDell_hist_7[self.index_:self.index__, :].copy_(self.keep_dDell_7.expand_as(self.dDell_hist_7[self.index_:self.index__, :]))
                            self.event_7.record()
                        with self.cuda_stream(self.stream_8):
                            self.dDell_hist_8[self.index_:self.index__, :].copy_(self.keep_dDell_8.expand_as(self.dDell_hist_8[self.index_:self.index__, :]))
                            self.event_8.record()
                        self.event_1.synchronize()
                        self.event_2.synchronize()
                        self.event_3.synchronize()
                    ...
                    self.index_ = self.latest + 1
                    if self.num_GPU == 1 or self.num_GPU == 0:
                        self.dDell_hist[self.index_:self.num, :].copy_(self.keep_dDell.expand_as(self.dDell_hist[self.index_:self.num, :]))
                        self.dDell_hist[0:self.index, :].copy_(self.keep_dDell.expand_as(self.dDell_hist[0:self.index, :]))
                    elif self.num_GPU == 2:
                        with self.cuda_stream(self.stream_1):
                            self.dDell_hist_1[self.index_:self.num, :].copy_(self.keep_dDell_1.expand_as(self.dDell_hist_1[self.index_:self.num, :]))
                            self.dDell_hist_1[0:self.index, :].copy_(self.keep_dDell_1.expand_as(self.dDell_hist_1[0:self.index, :]))
                            self.event_1.record()
                        with self.cuda_stream(self.stream_2):
                            self.dDell_hist_2[self.index_:self.num, :].copy_(self.keep_dDell_2.expand_as(self.dDell_hist_2[self.index_:self.num, :]))
                            self.dDell_hist_2[0:self.index, :].copy_(self.keep_dDell_2.expand_as(self.dDell_hist_2[0:self.index, :]))
                            self.event_2.record()
                        self.event_1.synchronize()
                        self.event_2.synchronize()
                    elif self.num_GPU == 4:
                        ...
                        with self.cuda_stream(self.stream_2):
                            ...
                            self.event_2.record()
                        with self.cuda_stream(self.stream_3):
                            self.dDell_hist_3[self.index_:self.num, :].copy_(self.keep_dDell_3.expand_as(self.dDell_hist_3[self.index_:self.num, :]))
                            self.dDell_hist_3[0:self.index, :].copy_(self.keep_dDell_3.expand_as(self.dDell_hist_3[0:self.index, :]))
                            self.event_3.record()
                        with self.cuda_stream(self.stream_4):
                            self.dDell_hist_4[self.index_:self.num, :].copy_(self.keep_dDell_4.expand_as(self.dDell_hist_4[self.index_:self.num, :]))
                            self.dDell_hist_4[0:self.index, :].copy_(self.keep_dDell_4.expand_as(self.dDell_hist_4[0:self.index, :]))
                            self.event_4.record()
                        self.event_1.synchronize()
                        self.event_2.synchronize()
                        self.event_3.synchronize()
                        ...
                        with self.cuda_stream(self.stream_4):
                            ...
                            self.event_4.record()
                        with self.cuda_stream(self.stream_5):
                            self.dDell_hist_5[self.index_:self.num, :].copy_(self.keep_dDell_5.expand_as(self.dDell_hist_5[self.index_:self.num, :]))
                            self.dDell_hist_5[0:self.index, :].copy_(self.keep_dDell_5.expand_as(self.dDell_hist_5[0:self.index, :]))
                            self.event_5.record()
                        with self.cuda_stream(self.stream_6):
                            self.dDell_hist_6[self.index_:self.num, :].copy_(self.keep_dDell_6.expand_as(self.dDell_hist_6[self.index_:self.num, :]))
                            self.dDell_hist_6[0:self.index, :].copy_(self.keep_dDell_6.expand_as(self.dDell_hist_6[0:self.index, :]))
                            self.event_2.record()
                        with self.cuda_stream(self.stream_7):
                            self.dDell_hist_7[self.index_:self.num, :].copy_(self.keep_dDell_7.expand_as(self.dDell_hist_7[self.index_:self.num, :]))
                            self.dDell_hist_7[0:self.index, :].copy_(self.keep_dDell_7.expand_as(self.dDell_hist_7[0:self.index, :]))
                            self.event_3.record()
                        with self.cuda_stream(self.stream_8):
                            self.dDell_hist_8[self.index_:self.num, :].copy_(self.keep_dDell_8.expand_as(self.dDell_hist_8[self.index_:self.num, :]))
                            self.dDell_hist_8[0:self.index, :].copy_(self.keep_dDell_8.expand_as(self.dDell_hist_8[0:self.index, :]))
                            self.event_4.record()
                        self.event_1.synchronize()
                        self.event_2.synchronize()
                        self.event_3.synchronize()
        ...
        if dt0 >= self.Tw:
            ...
            if self.num_GPU == 1 or self.num_GPU == 0:
                ...
                with self.cuda_stream(self.stream_4):
                    ...
                    self.event_4.record()
                with self.cuda_stream(self.stream_5):
                    self.k_inst_5 = self.kernel_5[self.num-self.inst:self.num, :].sum(dim=0)
                    self.k_inst_5 = self.k_inst_5.to('cuda:0', non_blocking=True)
                    self.event_5.record()
                with self.cuda_stream(self.stream_6):
                    self.k_inst_6 = self.kernel_6[self.num-self.inst:self.num, :].sum(dim=0)
                    self.k_inst_6 = self.k_inst_6.to('cuda:0', non_blocking=True)
                    self.event_6.record()
                with self.cuda_stream(self.stream_7):
                    self.k_inst_7 = self.kernel_7[self.num-self.inst:self.num, :].sum(dim=0)
                    self.k_inst_7 = self.k_inst_7.to('cuda:0', non_blocking=True)
                    self.event_7.record()
                with self.cuda_stream(self.stream_8):
                    self.k_inst_8 = self.kernel_8[self.num-self.inst:self.num, :].sum(dim=0)
                    self.k_inst_8 = self.k_inst_8.to('cuda:0', non_blocking=True)
                    self.event_8.record()
                self.event_1.synchronize()
                self.event_2.synchronize()
                self.event_3.synchronize()
                ...
                self.event_8.synchronize()
                self.K_inst = self.tr_cat((self.k_inst_1, self.k_inst_2,
                                           self.k_inst_3, self.k_inst_4,
                                           self.k_inst_5, self.k_inst_6,
                                           self.k_inst_7, self.k_inst_8,), dim=0) * d_Dell
                return self.K_inst
            ...
            if self.index >= 0:
                if self.num_GPU == 1 or self.num_GPU == 0:
                    ...
                    with self.cuda_stream(self.stream_2):
                        ...
                        self.time_conv += self.start.elapsed_time(self.end)
                        
                        self.start.record()
                        self.K2 = self.K2.to('cuda:0', non_blocking=True)
                        self.end.record()
                        self.end.synchronize()
                        self.time_transp += self.start.elapsed_time(self.end)
                    ...
                    with self.cuda_stream(self.stream_4):
                        ...
                        self.event_4.record()
                    with self.cuda_stream(self.stream_5):
                        self.K5 = (self.kernel_5[0:self.num-self.inst, :] * self.dDell_hist_5[self.index:self.latest+1, :]).sum(dim=0)
                        self.K5 = self.K5.to('cuda:0', non_blocking=True)
                        self.event_5.record()
                    with self.cuda_stream(self.stream_6):
                        self.K6 = (self.kernel_6[0:self.num-self.inst, :] * self.dDell_hist_6[self.index:self.latest+1, :]).sum(dim=0)
                        self.K6 = self.K6.to('cuda:0', non_blocking=True)
                        self.event_6.record()
                    with self.cuda_stream(self.stream_7):
                        self.K7 = (self.kernel_7[0:self.num-self.inst, :] * self.dDell_hist_7[self.index:self.latest+1, :]).sum(dim=0)
                        self.K7 = self.K7.to('cuda:0', non_blocking=True)
                        self.event_7.record()
                    with self.cuda_stream(self.stream_8):
                        self.K8 = (self.kernel_8[0:self.num-self.inst, :] * self.dDell_hist_8[self.index:self.latest+1, :]).sum(dim=0)
                        self.K8 = self.K8.to('cuda:0', non_blocking=True)
                        self.event_8.record()
                    self.event_1.synchronize()
                    self.event_2.synchronize()
                    self.event_3.synchronize()
                ...
                if self.num_GPU == 1 or self.num_GPU == 0:
                    ...
                    with self.cuda_stream(self.stream_4):
                        ...
                        self.event_4.record()
                    with self.cuda_stream(self.stream_5):
                        self.K5 = (self.kernel_5[0:-self.index, :] * self.dDell_hist_5[self.index+self.num:self.num, :]).sum(dim=0)
                        self.K5 += (self.kernel_5[-self.index:self.num-self.inst, :] * self.dDell_hist_5[0:self.latest+1, :]).sum(dim=0)
                        self.K5 = self.K5.to('cuda:0', non_blocking=True)
                        self.event_5.record()
                    with self.cuda_stream(self.stream_6):
                        self.K6 = (self.kernel_6[0:-self.index, :] * self.dDell_hist_6[self.index+self.num:self.num, :]).sum(dim=0)
                        self.K6 += (self.kernel_6[-self.index:self.num-self.inst, :] * self.dDell_hist_6[0:self.latest+1, :]).sum(dim=0)
                        self.K6 = self.K6.to('cuda:0', non_blocking=True)
                        self.event_6.record()
                    with self.cuda_stream(self.stream_7):
                        self.K7 = (self.kernel_7[0:-self.index, :] * self.dDell_hist_7[self.index+self.num:self.num, :]).sum(dim=0)
                        self.K7 += (self.kernel_7[-self.index:self.num-self.inst, :] * self.dDell_hist_7[0:self.latest+1, :]).sum(dim=0)
                        self.K7 = self.K7.to('cuda:0', non_blocking=True)
                        self.event_7.record()
                    with self.cuda_stream(self.stream_8):
                        self.K8 = (self.kernel_8[0:-self.index, :] * self.dDell_hist_8[self.index+self.num:self.num, :]).sum(dim=0)
                        self.K8 += (self.kernel_8[-self.index:self.num-self.inst, :] * self.dDell_hist_8[0:self.latest+1, :]).sum(dim=0)
                        self.K8 = self.K8.to('cuda:0', non_blocking=True)
                        self.event_8.record()
                    self.event_1.synchronize()
                    self.event_2.synchronize()
